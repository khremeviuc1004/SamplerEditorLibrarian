...
export class SamplerInMemorySampleMapper
  extends SamplerMapperBase
  implements SamplerHeaderMapper<Sample>
{
  ...
  mapFromUIDataByIndex(index: number, uiData: number): number[] {
    ...
    switch (index) {
      ...
      case 66:
      case 78:
        const results = this.convertFromLoopLengthIncludingFraction(uiData);
  ...
  mapToSysexData(sample: Sample): number[] {
    ...
    this.convertNameToSamplerSysexName(sample.name).forEach(
    ...
    data[40] = result[2];
    data[41] = result[3];
    let results = this.convertFromLoopLengthIncludingFraction(
      sample.loop1.loopLength,
    );
    data[42] = results[0][0];
    data[43] = results[0][1];
    data[44] = results[1][0];
    ...
    data[52] = result[2];
    data[53] = result[3];
    results = this.convertFromLoopLengthIncludingFraction(
      sample.loop2.loopLength,
    );
    data[54] = results[0][0];
    data[55] = results[0][1];
    data[56] = results[1][0];
  ...
  public mapFromSysexData(data: Array<number>): Sample {
    ...
    this.mapFromSysexDataUsingExistingProgram(data, sample);
  ...
  public mapFromSysexDataUsingExistingProgram(
    ...
    sample.tune = this.convertToPlusOrMinusFiftyIncludingFraction(
      data[21],
      data[20],
    );

    sample.sampleLength =
      data[26] | (data[27] << 8) | (data[28] << 16) | (data[29] << 24);
    sample.startOffset =
      data[30] | (data[31] << 8) | (data[32] << 16) | (data[33] << 24);
    sample.playLength =
      data[34] | (data[35] << 8) | (data[36] << 16) | (data[37] << 24);

    sample.loop1.loopStart =
      data[38] | (data[39] << 8) | (data[40] << 16) | (data[41] << 24);
    sample.loop1.loopLength = this.convertToLoopLengthIncludingFraction(
      [data[42], data[43]],
      [data[44], data[45], data[46], data[47]],
    );
    sample.loop1.dwellTime = data[48] | (data[49] << 8);

    sample.loop2.loopStart =
      data[50] | (data[51] << 8) | (data[52] << 16) | (data[53] << 24);
    sample.loop2.loopLength = this.convertToLoopLengthIncludingFraction(
      [data[54], data[55]],
      [data[56], data[57], data[58], data[59]],
    );
    sample.loop2.dwellTime = data[60] | (data[61] << 8);

    sample.loop3.loopStart =
      data[62] | (data[63] << 8) | (data[64] << 16) | (data[65] << 24);
    sample.loop3.loopLength = this.convertToLoopLengthIncludingFraction(
      [data[66], data[67]],
      [data[68], data[69], data[70], data[71]],
    );
    sample.loop3.dwellTime = data[72] | (data[73] << 8);

    sample.loop4.loopStart =
      data[74] | (data[75] << 8) | (data[76] << 16) | (data[77] << 24);
    sample.loop4.loopLength = this.convertToLoopLengthIncludingFraction(
      [data[78], data[79]],
      [data[80], data[81], data[82], data[83]],
    );
    sample.loop4.dwellTime = data[84] | (data[85] << 8);

    sample.loop1.relativeLoopFactors =
      data[86] | (data[87] << 8) | (data[88] << 16) | (data[89] << 24);
    sample.loop2.relativeLoopFactors =
      data[98] | (data[99] << 8) | (data[100] << 16) | (data[101] << 24);
    sample.loop3.relativeLoopFactors =
      data[110] | (data[111] << 8) | (data[112] << 16) | (data[113] << 24);
    sample.loop4.relativeLoopFactors =
      data[122] | (data[123] << 8) | (data[124] << 16) | (data[125] << 24);
