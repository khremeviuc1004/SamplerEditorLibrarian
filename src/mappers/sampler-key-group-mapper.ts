...
export class SamplerInMemoryKeyGroupMapper
  extends SamplerMapperBase
  implements SamplerHeaderMapper<KeyGroup>
{
  ...
  mapFromUIDataByIndex(index: number, uiData: number): number[] {
    ...
    if (data.length == 2) {
      console.log(
        'Keygroup mapper - mapFromUIDataByIndex - converted to: ',
        data[0],
        data[1],
      );
    } else {
  ...
  mapToSysexData(keyGroup: KeyGroup): number[] {
    ...
    const [fractionPart, wholePart] =
      this.convertFromPlusOrMinusFiftyIncludingFraction(keyGroup.span.tune);
    data[5] = fractionPart;
    data[6] = wholePart;
    ...
    data[16] = this.convertFromPlusOrMinusFifty(
      keyGroup.envelope1.velocityModulationOfAttack,
    );
    data[17] = this.convertFromPlusOrMinusFifty(
      keyGroup.envelope1.velocityModulationOfRelease,
    );
    data[18] = this.convertFromPlusOrMinusFifty(
      keyGroup.envelope1.velocityOffModulationOfRelease,
    );
    data[19] = this.convertFromPlusOrMinusFifty(
      keyGroup.envelope1.keyModulationOfDecayAndRelease,
    );
    ...
    data[24] = this.convertFromPlusOrMinusFifty(
      keyGroup.envelope2.velocityModulationOfRate1,
    );
    data[25] = this.convertFromPlusOrMinusFifty(
      keyGroup.envelope2.velocityModulationOfRate4,
    );
    data[26] = this.convertFromPlusOrMinusFifty(
      keyGroup.envelope2.velocityOffModulationOfRate4,
    );
    data[27] = this.convertFromPlusOrMinusFifty(
      keyGroup.envelope2.keyModulationOfRate2AndRate4,
    );
    data[28] = this.convertFromPlusOrMinusFifty(
      keyGroup.envelope2.velocityModulationOfEnvelope,
    );

    data[30] = keyGroup.velocityCrossFade ? 1 : 0;

    this.convertNameToSamplerSysexName(keyGroup.zone1.sampleName).forEach(
      (value, index) => (data[34 + index] = value),
    );
    data[46] = keyGroup.zone1.velocityLow;
    data[47] = keyGroup.zone1.velocityHigh;
    const [fractionPartZone1, wholePartZone1] =
      this.convertFromPlusOrMinusFiftyIncludingFraction(keyGroup.zone1.tune);
    ...
    this.convertNameToSamplerSysexName(keyGroup.zone2.sampleName).forEach(
      (value, index) => (data[58 + index] = value),
    );
    data[70] = keyGroup.zone2.velocityLow;
    data[71] = keyGroup.zone2.velocityHigh;
    const [fractionPartZone2, wholePartZone2] =
      this.convertFromPlusOrMinusFiftyIncludingFraction(keyGroup.zone2.tune);
    ...
    this.convertNameToSamplerSysexName(keyGroup.zone3.sampleName).forEach(
      (value, index) => (data[82 + index] = value),
    );
    data[94] = keyGroup.zone3.velocityLow;
    data[95] = keyGroup.zone3.velocityHigh;
    const [fractionPartZone3, wholePartZone3] =
      this.convertFromPlusOrMinusFiftyIncludingFraction(keyGroup.zone3.tune);
    ...
    this.convertNameToSamplerSysexName(keyGroup.zone4.sampleName).forEach(
      (value, index) => (data[106 + index] = value),
    );
    data[118] = keyGroup.zone4.velocityLow;
    data[119] = keyGroup.zone4.velocityHigh;
    const [fractionPartZone4, wholePartZone4] =
      this.convertFromPlusOrMinusFiftyIncludingFraction(keyGroup.zone4.tune);
    ...
    const velToStartPosAdjZone1 = this.convertFromPlusOrMinusNineNineNine(
      keyGroup.zone1.velToStartPosAdj,
    );
    data[140] = velToStartPosAdjZone1[0];
    data[141] = velToStartPosAdjZone1[1];
    const velToStartPosAdjZone2 = this.convertFromPlusOrMinusNineNineNine(
      keyGroup.zone2.velToStartPosAdj,
    );
    data[142] = velToStartPosAdjZone2[0];
    data[143] = velToStartPosAdjZone2[1];
    const velToStartPosAdjZone3 = this.convertFromPlusOrMinusNineNineNine(
      keyGroup.zone3.velToStartPosAdj,
    );
    data[144] = velToStartPosAdjZone3[0];
    data[145] = velToStartPosAdjZone3[1];
    const velToStartPosAdjZone4 = this.convertFromPlusOrMinusNineNineNine(
      keyGroup.zone4.velToStartPosAdj,
    );
    ...
    data[150] = this.convertFromPlusOrMinusFifty(
      keyGroup.pitchModulationByLFO1,
    );

    data[151] = this.convertFromPlusOrMinusFifty(
      keyGroup.filter1.freqModulationInput1Amount,
    );
    data[152] = this.convertFromPlusOrMinusFifty(
      keyGroup.filter1.freqModulationInput2Amount,
    );
    data[153] = this.convertFromPlusOrMinusFifty(
      keyGroup.filter1.freqModulationInput3Amount,
    );

    data[154] = this.convertFromPlusOrMinusFifty(
      keyGroup.pitchModulationInputAmount,
    );

    data[155] = this.convertFromPlusOrMinusFifty(
      keyGroup.loudnessModulationInputAmount,
    );
    ...
    data[174] = this.convertFromPlusOrMinusFifty(
      keyGroup.filter2.freqModulationInput1Amount,
    );
    data[175] = this.convertFromPlusOrMinusFifty(
      keyGroup.filter2.freqModulationInput2Amount,
    );
    data[176] = this.convertFromPlusOrMinusFifty(
      keyGroup.filter2.freqModulationInput3Amount,
    );

    data[177] = keyGroup.filter2.frequency;
    data[178] = this.convertFromPlusOrMinusTwentyFour(
      keyGroup.filter2.keyFollow,
    );
    ...
    data[187] = this.convertFromPlusOrMinusFifty(
      keyGroup.envelope3.velocityModulationOfRate1,
    );
    data[188] = this.convertFromPlusOrMinusFifty(
      keyGroup.envelope3.velocityModulationOfRate4,
    );
    data[189] = this.convertFromPlusOrMinusFifty(
      keyGroup.envelope3.velocityOffModulationOfRate4,
    );
    data[190] = this.convertFromPlusOrMinusFifty(
      keyGroup.envelope3.keyModulationOfRate2AndRate4,
    );
    data[191] = this.convertFromPlusOrMinusFifty(
      keyGroup.envelope3.velocityModulationOfEnvelope,
    );
  ...
  public mapFromSysexData(data: Array<number>): KeyGroup {
    ...
    this.mapFromSysexDataUsingExistingKeygroup(data, keyGroup);

    return keyGroup;
  }

  public mapFromSysexDataUsingExistingKeygroup(
    data: Array<number>,
    keyGroup: KeyGroup,
  ) {
    ...
    keyGroup.span.tune = this.convertToPlusOrMinusFiftyIncludingFraction(
      data[6],
      data[5],
    );
    ...
    keyGroup.envelope1.velocityModulationOfAttack =
      this.convertToPlusOrMinusFifty(data[16]);
    keyGroup.envelope1.velocityModulationOfRelease =
      this.convertToPlusOrMinusFifty(data[17]);
    keyGroup.envelope1.velocityOffModulationOfRelease =
      this.convertToPlusOrMinusFifty(data[18]);
    keyGroup.envelope1.keyModulationOfDecayAndRelease =
      this.convertToPlusOrMinusFifty(data[19]);
    ...
    keyGroup.envelope2.velocityModulationOfRate1 =
      this.convertToPlusOrMinusFifty(data[24]);
    keyGroup.envelope2.velocityModulationOfRate4 =
      this.convertToPlusOrMinusFifty(data[25]);
    keyGroup.envelope2.velocityOffModulationOfRate4 =
      this.convertToPlusOrMinusFifty(data[26]);
    keyGroup.envelope2.keyModulationOfRate2AndRate4 =
      this.convertToPlusOrMinusFifty(data[27]);
    keyGroup.envelope2.velocityModulationOfEnvelope =
      this.convertToPlusOrMinusFifty(data[28]);

    keyGroup.velocityCrossFade = data[30] == 0 ? false : true;

    keyGroup.zone1.sampleName = this.convertSamplerSysexNameToName(
      data.slice(34, 46),
    );
    keyGroup.zone1.velocityLow = data[46];
    keyGroup.zone1.velocityHigh = data[47];
    keyGroup.zone1.tune = this.convertToPlusOrMinusFiftyIncludingFraction(
      data[49],
      data[48],
    );
    ...
    keyGroup.zone2.sampleName = this.convertSamplerSysexNameToName(
      data.slice(58, 70),
    );
    keyGroup.zone2.velocityLow = data[70];
    keyGroup.zone2.velocityHigh = data[71];
    keyGroup.zone2.tune = this.convertToPlusOrMinusFiftyIncludingFraction(
      data[73],
      data[72],
    );
    ...
    keyGroup.zone3.sampleName = this.convertSamplerSysexNameToName(
      data.slice(82, 94),
    );
    keyGroup.zone3.velocityLow = data[94];
    keyGroup.zone3.velocityHigh = data[95];
    keyGroup.zone3.tune = this.convertToPlusOrMinusFiftyIncludingFraction(
      data[97],
      data[96],
    );
    ...
    keyGroup.zone4.sampleName = this.convertSamplerSysexNameToName(
      data.slice(106, 118),
    );
    keyGroup.zone4.velocityLow = data[118];
    keyGroup.zone4.velocityHigh = data[119];
    keyGroup.zone4.tune = this.convertToPlusOrMinusFiftyIncludingFraction(
      data[121],
      data[120],
    );
    ...
    keyGroup.zone1.velToStartPosAdj = this.convertToPlusOrMinusNineNineNine(
      data[140] | (data[141] << 8),
    );
    keyGroup.zone2.velToStartPosAdj = this.convertToPlusOrMinusNineNineNine(
      data[142] | (data[143] << 8),
    );
    keyGroup.zone3.velToStartPosAdj = this.convertToPlusOrMinusNineNineNine(
      data[144] | (data[145] << 8),
    );
    keyGroup.zone4.velToStartPosAdj = this.convertToPlusOrMinusNineNineNine(
      data[146] | (data[147] << 8),
    );
    ...
    keyGroup.filter1.freqModulationInput1Amount =
      this.convertToPlusOrMinusFifty(data[151]);
    keyGroup.filter1.freqModulationInput2Amount =
      this.convertToPlusOrMinusFifty(data[152]);
    keyGroup.filter1.freqModulationInput3Amount =
      this.convertToPlusOrMinusFifty(data[153]);

    keyGroup.pitchModulationInputAmount = this.convertToPlusOrMinusFifty(
      data[154],
    );

    keyGroup.loudnessModulationInputAmount = this.convertToPlusOrMinusFifty(
      data[155],
    );
    ...
    keyGroup.filter2.freqModulationInput1Amount =
      this.convertToPlusOrMinusFifty(data[174]);
    keyGroup.filter2.freqModulationInput2Amount =
      this.convertToPlusOrMinusFifty(data[175]);
    keyGroup.filter2.freqModulationInput3Amount =
      this.convertToPlusOrMinusFifty(data[176]);
    ...
    keyGroup.envelope3.velocityModulationOfRate1 =
      this.convertToPlusOrMinusFifty(data[187]);
    keyGroup.envelope3.velocityModulationOfRate4 =
      this.convertToPlusOrMinusFifty(data[188]);
    keyGroup.envelope3.velocityOffModulationOfRate4 =
      this.convertToPlusOrMinusFifty(data[189]);
    keyGroup.envelope3.keyModulationOfRate2AndRate4 =
      this.convertToPlusOrMinusFifty(data[190]);
    keyGroup.envelope3.velocityModulationOfEnvelope =
      this.convertToPlusOrMinusFifty(data[191]);
  }
}
