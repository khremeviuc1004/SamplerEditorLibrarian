...
NEGATIVE_FRACTION_FROM_SYSEX_LOOKUP.set(231, -0.10)
...
NEGATIVE_FRACTION_FROM_SYSEX_LOOKUP.set(205, -0.20)
...
NEGATIVE_FRACTION_FROM_SYSEX_LOOKUP.set(180, -0.30)
...
NEGATIVE_FRACTION_FROM_SYSEX_LOOKUP.set(154, -0.40)
...
NEGATIVE_FRACTION_FROM_SYSEX_LOOKUP.set(128, -0.50)
...
NEGATIVE_FRACTION_FROM_SYSEX_LOOKUP.set(103, -0.60)
...
NEGATIVE_FRACTION_FROM_SYSEX_LOOKUP.set(77, -0.70)
...
NEGATIVE_FRACTION_FROM_SYSEX_LOOKUP.set(52, -0.80)
...
NEGATIVE_FRACTION_FROM_SYSEX_LOOKUP.set(26, -0.90)
...
SYSEX_TO_NEGATIVE_FRACTION_LOOKUP.set(-0.10, 231)
...
SYSEX_TO_NEGATIVE_FRACTION_LOOKUP.set(-0.20, 205)
...
SYSEX_TO_NEGATIVE_FRACTION_LOOKUP.set(-0.30, 180)
...
SYSEX_TO_NEGATIVE_FRACTION_LOOKUP.set(-0.40, 154)
...
SYSEX_TO_NEGATIVE_FRACTION_LOOKUP.set(-0.50, 128)
...
SYSEX_TO_NEGATIVE_FRACTION_LOOKUP.set(-0.60, 103)
...
SYSEX_TO_NEGATIVE_FRACTION_LOOKUP.set(-0.70, 77)
...
SYSEX_TO_NEGATIVE_FRACTION_LOOKUP.set(-0.80, 52)
...
SYSEX_TO_NEGATIVE_FRACTION_LOOKUP.set(-0.90, 26)
...
POSITIVE_FRACTION_FROM_SYSEX_LOOKUP.set(25, 0.10)
...
POSITIVE_FRACTION_FROM_SYSEX_LOOKUP.set(51, 0.20)
...
POSITIVE_FRACTION_FROM_SYSEX_LOOKUP.set(76, 0.30)
...
POSITIVE_FRACTION_FROM_SYSEX_LOOKUP.set(102, 0.40)
...
POSITIVE_FRACTION_FROM_SYSEX_LOOKUP.set(128, 0.50)
...
POSITIVE_FRACTION_FROM_SYSEX_LOOKUP.set(153, 0.60)
...
POSITIVE_FRACTION_FROM_SYSEX_LOOKUP.set(179, 0.70)
...
POSITIVE_FRACTION_FROM_SYSEX_LOOKUP.set(204, 0.80)
...
POSITIVE_FRACTION_FROM_SYSEX_LOOKUP.set(230, 0.90)
...
SYSEX_TO_POSITIVE_FRACTION_LOOKUP.set(0.10, 25)
...
SYSEX_TO_POSITIVE_FRACTION_LOOKUP.set(0.20, 51)
...
SYSEX_TO_POSITIVE_FRACTION_LOOKUP.set(0.30, 76)
...
SYSEX_TO_POSITIVE_FRACTION_LOOKUP.set(0.40, 102)
...
SYSEX_TO_POSITIVE_FRACTION_LOOKUP.set(0.50, 128)
...
SYSEX_TO_POSITIVE_FRACTION_LOOKUP.set(0.60, 153)
...
SYSEX_TO_POSITIVE_FRACTION_LOOKUP.set(0.70, 179)
...
SYSEX_TO_POSITIVE_FRACTION_LOOKUP.set(0.80, 204)
...
SYSEX_TO_POSITIVE_FRACTION_LOOKUP.set(0.90, 230)
...
export class SamplerMapperBase {
    ...
    convertSampleSysexModulationSourceType(modulationSourceType: number): ModulationSourceType {
    ...
    convertSamplerSysexNameToName(name_bytes: Array<number>): string {
        ...
        for (let [index, name_byte] of name_bytes.entries()) {
    ...
    convertNameToSamplerSysexName(name: string): Array<number> {
        ...
        Array.from(name.toUpperCase()).forEach((value) => {
            let index = SAMPLER_CHAR_MAP.findIndex((mapValue) => value === mapValue)

            if (index >= 0) {
                name_bytes.push(index)
            }
            else {
                throw new NotAcceptableException("Could not find item name character to sysex name character")
            }
        })
    ...
    convertFractionalPartOfValue(wholeByte: number, fractionByte: number): number {
    ...
    convertToPlusOrMinusFiftyWithFraction(wholeByte: number, fractionByte: number): number {
        if (wholeByte >= 206 && fractionByte == 0) {
    ...
    convertToPlusOrMinusFiftyIncludingFraction(wholeByte: number, fractionByte: number): number {
        if (wholeByte >= 206 && fractionByte == 0) {
            return wholeByte - 256
        }
        else if (wholeByte >= 206 && fractionByte > 0) {
            return wholeByte - 255 + NEGATIVE_FRACTION_FROM_SYSEX_LOOKUP.get(fractionByte)
        }
    ...
    convertFromPlusOrMinusFiftyIncludingFraction(tuning: number): Array<number> {
        ...
        let wholePart = Math.trunc(tuning)
        let fractionalPart = Math.round((tuning % 1) * 100) / 100
    ...
    convertToLoopLengthIncludingFraction(fractionNumber: number[], wholeNumber: number[]): number {
        let loopLengthMinor =  Math.round(((fractionNumber[0] | (fractionNumber[1] << 8))) / (65535 / 999))
        let loopLength = wholeNumber[0] | (wholeNumber[1] << 8) | (wholeNumber[2] << 16) | (wholeNumber[3] << 24)
        return loopLength + (loopLengthMinor / 1000)
    }

    convertFromLoopLengthIncludingFraction(loopLength: number): Array<Array<number>> {
        let wholePart = Math.trunc(loopLength)
        ...
        return [[fractionalPart & 255, fractionalPart >> 8], [wholePart & 255, (wholePart >> 8) & 255, (wholePart >> 16) & 255, (wholePart >> 24) & 255]]
    }
}